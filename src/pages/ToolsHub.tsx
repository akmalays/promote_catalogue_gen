      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-10 md:py-14">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-3 py-2 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-stone-900/10 focus:bg-white"
              />
            </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-16 bg-white border border-stone-200 rounded-2xl">
            <p className="text-sm text-stone-500">Tidak ada tools yang cocok dengan filter.</p>
          </div>
        ) : (
            ...
            {filtered.map(tool => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'group p-5 bg-white border border-stone-200 rounded-2xl text-left transition-colors',
                    tool.available
                ...
                >
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <div className="flex flex-wrap items-center gap-1 justify-end">
                      <span className="text-[10px] font-medium text-stone-500 px-1.5 py-0.5 bg-stone-100 rounded">
                        {categoryLabel[tool.category]}
                      </span>
                      {tool.forBusiness.map(b => (
                        <span
                          key={b}
                          className="text-[10px] font-medium text-stone-500 px-1.5 py-0.5 bg-stone-100 rounded"
                        >
                          {businessLabel[b]}
                        </span>
                      ))}
                  ...
                  {tool.available ? (
                    <span className="inline-flex items-center gap-1.5 text-xs font-medium text-stone-700 group-hover:text-stone-900">
                      Buka tools <ArrowRight className="w-3.5 h-3.5" />
                    </span>
                  ) : (
                    <span className="text-xs font-medium text-stone-400">Segera hadir</span>
                  )}
                </button>
              );
            })}
