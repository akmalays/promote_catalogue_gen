...
/**
 * Simple Margin & Markup Calculator (light + dark mode aware).
 */
export default function MarginCalculator() {
  ...
  const calculatedPrice = useMemo(() => {
    if (cost === 0) return 0;
    if (mode === 'markup') return Math.round(cost * (1 + percent / 100));
    if (percent >= 100) return 0;
    return Math.round(cost / (1 - percent / 100));
  }, [cost, percent, mode]);

  // Reverse: cost + sellingPrice → margin & markup
    ...
    >
      <div className="space-y-4">
        ...
        <div className="bg-white dark:bg-stone-900/50 dark:backdrop-blur-sm border border-stone-200 dark:border-stone-800 rounded-2xl p-5 md:p-6">
          <div className="grid grid-cols-2 gap-1.5 p-1 bg-stone-100 dark:bg-stone-800/60 rounded-lg mb-5">
            <button
          ...
          <div className="flex items-start gap-2.5 p-3 bg-stone-50 dark:bg-stone-800/40 border border-stone-200 dark:border-stone-700/60 rounded-lg mb-5">
            ...
            <p className="text-xs text-stone-600 dark:text-stone-300 leading-relaxed">
              {mode === 'markup' ? (
                <>
                  <strong className="text-stone-900 dark:text-white">Markup</strong> = profit dibagi modal. Contoh: modal Rp 10.000, markup 50% → harga jual Rp 15.000.
                </>
              ) : (
                <>
                  <strong className="text-stone-900 dark:text-white">Margin</strong> = profit dibagi harga jual. Contoh: modal Rp 10.000, margin 50% → harga jual Rp 20.000 (margin lebih besar dari markup pada angka yang sama).
                </>
              )}
            </p>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1 px-2.5 py-1 bg-stone-100 dark:bg-stone-800 rounded-md tabular-nums">
                  <input
                    ...
                    className="w-12 bg-transparent text-sm font-semibold text-right focus:outline-none text-stone-900 dark:text-white"
                  />
          ...
          {cost > 0 && calculatedPrice > 0 && (
            <div className="mt-5 p-4 bg-stone-900 dark:bg-stone-800 dark:border dark:border-stone-700 text-white rounded-xl">
        ...
        <div className="bg-white dark:bg-stone-900/50 dark:backdrop-blur-sm border border-stone-200 dark:border-stone-800 rounded-2xl p-5 md:p-6">
          <h2 className="text-base md:text-lg font-semibold mb-1 text-stone-900 dark:text-white">Atau cek margin dari harga jual yang sudah ada</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
            <div>
              <label className="text-xs font-medium text-stone-600 dark:text-stone-400 block mb-1.5">Modal</label>
              <CurrencyInput value={cost} onChange={setCost} placeholder="0" />
            </div>
            <div>
              <label className="text-xs font-medium text-stone-600 dark:text-stone-400 block mb-1.5">Harga jual</label>
          ...
          {reverseAnalysis ? (
            ...
            <p className="text-xs text-stone-500 dark:text-stone-400 italic">Lengkapi modal & harga jual untuk lihat hasilnya.</p>
          )}
        </div>

        {/* Tips */}
        <div className="bg-white dark:bg-stone-900/50 dark:backdrop-blur-sm border border-stone-200 dark:border-stone-800 rounded-2xl p-5 md:p-6">
          <h2 className="text-base md:text-lg font-semibold mb-3 text-stone-900 dark:text-white">Panduan margin per kategori</h2>
...
function Stat({ label, value, positive }: { label: string; value: string; positive?: boolean }) {
  return (
    <div className="p-3 bg-stone-50 dark:bg-stone-800/40 border border-stone-200 dark:border-stone-700/60 rounded-xl">
      ...
      <p className={cn(
        'text-base font-bold tabular-nums text-stone-900 dark:text-white',
  ...
  return (
    <div className="flex items-center justify-between gap-3 p-3 bg-stone-50 dark:bg-stone-800/40 border border-stone-200 dark:border-stone-700/60 rounded-lg">
      <div className="min-w-0">
        <p className="text-xs font-semibold text-stone-900 dark:text-white truncate">{category}</p>
        <p className="text-[11px] text-stone-500 dark:text-stone-400 truncate">{note}</p>
      </div>
      <span className="text-xs font-bold text-stone-900 dark:text-white tabular-nums whitespace-nowrap">{range}</span>
    </div>
  );
}
